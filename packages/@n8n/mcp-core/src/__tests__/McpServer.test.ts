...
describe('McpServer', () => {
	...
	describe('pending response management', () => {
		it('should track and check pending responses', async () => {
			...
			const sessionManager = (mcpServer as unknown as { sessionManager: { registerSession: (s: string, srv: unknown, tr: unknown) => Promise<void> } }).sessionManager;
	...
	describe('handleWorkerResponse', () => {
		it('should handle list tools request marker', async () => {
			...
			const sessionManager = (mcpServer as unknown as {
				sessionManager: {
					registerSession: (s: string, srv: unknown, tr: unknown, tools?: unknown[]) => Promise<void>;
				};
			}).sessionManager;
			await sessionManager.registerSession(sessionId, server, transport, [createMockTool('test-tool')]);
