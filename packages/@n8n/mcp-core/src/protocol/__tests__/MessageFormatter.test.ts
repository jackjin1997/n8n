...
describe('MessageFormatter', () => {
	...
	describe('formatError', () => {
		it('should format error with isError flag set to true', () => {
			...
			expect(MessageFormatter.formatError(error)).toEqual({
				isError: true,
		...
		it('should handle error with special characters in message', () => {
			...
			expect(MessageFormatter.formatError(error)).toEqual({
				isError: true,
				content: [{ type: 'text', text: 'Error: Failed: "invalid" <value>' }],
			});
		});
		...
		it('should handle TypeError', () => {
			...
			expect(MessageFormatter.formatError(error)).toEqual({
				isError: true,
				content: [{ type: 'text', text: 'Error: Cannot read property of undefined' }],
			});
		});
		...
		it('should handle custom error subclass', () => {
			...
			expect(MessageFormatter.formatError(error)).toEqual({
				isError: true,
				content: [{ type: 'text', text: 'Error: Custom error message' }],
			});
		});
	});
});
