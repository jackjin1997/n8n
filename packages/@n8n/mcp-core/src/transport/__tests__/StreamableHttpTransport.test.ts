...
describe('StreamableHttpTransport', () => {
	describe('constructor', () => {
		it('should set transportType to streamableHttp', () => {
			...
			const transport = new StreamableHttpTransport({ sessionIdGenerator: () => 'test-id' }, response);

			expect(transport.transportType).toBe('streamableHttp');
		});

		it('should accept sessionIdGenerator option', () => {
			const response = createMockResponse();
		...
		it('should accept onsessioninitialized callback', () => {
			const response = createMockResponse();
